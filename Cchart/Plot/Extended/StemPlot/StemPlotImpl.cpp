...
CStemPlotImpl::CStemPlotImpl()
{
	...
	axis->SetLabelSuffix(_TEXT("ÎåÈÕ"));
...
void	CStemPlotImpl::GetPlotRange( double *xRange, double *yRange )
{
	...
	for(i=0; i<GetDataSetCount(); i++)
	{
		if(GetXYDataInfo()[i]->pDataSet->GetDataSize() > m_nMaxSize )
			m_nMaxSize = m_vpXYDataInfo[i]->pDataSet->GetDataSize();
	}
	xRange[0] = 1.0;
...
void	CStemPlotImpl::DrawSingleDataSet( int which, HDC hDC, RECT plotRect, double *xRange, double *yRange )
{
	...
	if( m_vpXYDataInfo[which]->bLighted || m_vpXYDataInfo[which]->bSelected )
	{
		linecolor = RGB(255 - GetRValue(linecolor), 255 - GetGValue(linecolor), 255 - GetBValue(linecolor));
	}
	
	hPen = CreatePen( m_vpXYDataInfo[which]->nPlotLineStyle, m_vpXYDataInfo[which]->nPlotLineSize, linecolor );
	hOldPen = (HPEN)SelectObject(hDC, hPen );
	hBrush	= CreateSolidBrush( m_vpXYDataInfo[which]->crPlotColor );
...
{
	if(!m_vpXYDataInfo[which]->bVisible)return false;
