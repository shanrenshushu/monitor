	...
	*/
	...
	if (x < m_nFixColCount || y < m_nFixRowCount)
	{
		...
		if(GetCellText(id))
		{
			...
			//_stprintf(sz, "%s", GetCellText(id));
			//DrawText(hdc, sz, _tcslen(sz), &rc,	DT_SINGLELINE | DT_CENTER | DT_VCENTER);
			return ;
		}
...
{
	CMemDC dc(hdc, pGrid->GetBkgBrush());
	...
	LPARAM lParam)
{
	...
	if (pGrid)
	{
		...
		case WM_ERASEBKGND:
			...
			break;
		case WM_SIZE:
			pGrid->OnSize(wParam, LOWORD(lParam), HIWORD(lParam));
			break;
		case WM_VSCROLL:
		...
		case WM_LBUTTONDOWN:
			...
			pGrid->OnLButtonDown(wParam, pt);
			break;
		case WM_LBUTTONUP:
			pt.x = (int) (short) LOWORD(lParam);
			pt.y = (int) (short) HIWORD(lParam);
			pGrid->OnLButtonUp(wParam, pt);
			break;
		case WM_LBUTTONDBLCLK:
			pt.x = (int) (short) LOWORD(lParam);
			pt.y = (int) (short) HIWORD(lParam);
			pGrid->OnLButtonDblClk(wParam, pt);
			break;
		case WM_RBUTTONDOWN:
			pt.x = (int) (short) LOWORD(lParam);
			pt.y = (int) (short) HIWORD(lParam);
			pGrid->OnRButtonDown(wParam, pt);
			break;
		case WM_KEYDOWN:
