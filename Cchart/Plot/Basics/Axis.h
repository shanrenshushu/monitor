...
protected:
	...
	//2018/1/2 csh
	bool		m_isTimeFormatX;
	string		m_strTimeFormat;
	tstring		m_strDataFormat;
public:
...
public:
	...
	// 设置子图时间坐标轴格式,"%Y%m%d%H%M%S",参考strftime函数
	void		SetAxisTimeFormat( const string &format){m_strTimeFormat = format;m_isTimeFormatX = true;}
	void		SetAxisDataFormat( const tstring &format){m_strDataFormat = format;}
};
