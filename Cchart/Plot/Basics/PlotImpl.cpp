...
void CPlotImpl::OnDraw(HDC hDC, RECT destRect)
{
	...
	if(m_bEdgeShow)
	{
		DrawPlotEdge(hDC, destRect);
//		RECT edgeRect = destRect;
//		DrawEdge(hDC, &edgeRect, m_nEdgeType, m_nEdgeFlag);
	}
...
void CPlotImpl::DrawTextInRect(HDC hDC, RECT rect, tstring title, LOGFONT logFont, int backmode, COLORREF color)
{
	if( title == _TEXT("") )return;
...
void CPlotImpl::DrawTitle(HDC hDC, RECT titleRect)
{
	...
	if(m_bLightTitle || m_bSelectTitle)
	{
		HPEN hPen;
		if(m_bSelectTitle)
		{
			hPen = CreatePen( PS_SOLID, m_nLightLineSize, m_crLightColor );
		}
		else
		{
			hPen = CreatePen( PS_SOLID, m_nLightLineSize, m_crSelectColor );
		}
...
{
	if( m_strSubtitle == _TEXT("") || !m_bSubtitleShow )return;
...
void	CPlotImpl::DrawPlotEdge( HDC hDC, RECT destRect )
{
	...
	COLORREF edgecolor = ChooseLineColor(m_crEdgeColor, m_bPlotLighted, m_bPlotSelected);
	int edgesize = ChooseLineWidth(m_nEdgeSize, m_bPlotLighted, m_bPlotSelected);
...
SIZE	CPlotImpl::GetTitleSize( HDC hDC )
{
	SIZE size={0,0};
...
{
	if( m_strTitle != _TEXT("") && m_bTitleShow )
	{
		SIZE		tmSize={0,0};
		HFONT		hOldFont,hFont;
		hFont = CreateFontIndirect(&m_lfTitleFont);
