...
void CAxisImpl::OnDraw(HDC hDC, RECT clientRect, RECT chartRect)
{
	...
	if(m_bSelected)
	{
		DrawFrame(hDC,clientRect,chartRect,m_pPlot->GetLigntLineStyle(), m_pPlot->GetLightLineSize(), m_pPlot->GetSelectColor());
	}
	else if(m_bLighted)
	{
		...
		case kLocationCenterHT:
			{
				...
				tickLabel = GetTickLabel( i );
				DrawText(hDC, tickLabel.c_str(),tickLabel.length(), (LPRECT)&textRect, textMode &= ~DT_SINGLELINE );
			}
			break;
		case kLocationLeft:
		case kLocationCenterVL:
...
SIZE	CAxisImpl::GetMinDisplaySize( HDC hDC )
{
	...
	//if( m_strTitle != "" )
	if( m_strTitle != _TEXT("") && m_nTitlePosition==kTitlePositionNormal)
	{
...
// tick labels.
tstring		CAxisImpl::GetTickLabel( int whichTick )
{
	tstring			tickLabel;
	if (m_isTimeFormatX)
	{
		...
		strftime(buf, 64, m_strTimeFormat.c_str(), timeinfo);  
#if defined(_UNICODE) || defined(UNICODE)
		tickLabel = StrToWStr(buf);
#else
		tickLabel = buf;
#endif
	}
	else if(!m_bReplaceLabel)
	{
		double			value = GetTickPosValue( whichTick );
		if (m_strDataFormat.empty())
		{
			tostringstream ostr;
			ostr<<value;
			tickLabel = ostr.str();
...
#if defined(_UNICODE) || defined(UNICODE)
			wchar_t buf[128]={0};
			swprintf_s(buf,128,m_strDataFormat.c_str(),value);
			tickLabel = buf;
			
#else
			char buf[128]={0};
			sprintf(buf,m_strDataFormat.c_str(),value);
			tickLabel = buf;
#endif
		}
