...
public:
	MatchResult MatchExact(const CHART * tbstring, CContext * pContext = 0) const;
	MatchResult MatchExact(const CHART * tbstring, int length, CContext * pContext = 0) const;
	MatchResult Match(const CHART * tbstring, int start = -1, CContext * pContext = 0) const;
	MatchResult Match(const CHART * tbstring, int length, int start, CContext * pContext = 0) const;
	MatchResult Match(CContext * pContext) const;
	CContext * PrepareMatch(const CHART * tbstring, int start = -1, CContext * pContext = 0) const;
	CContext * PrepareMatch(const CHART * tbstring, int length, int start, CContext * pContext = 0) const;
	CHART * Replace(const CHART * tbstring, const CHART * replaceto, int start = -1, int ntimes = -1, MatchResult * result = 0, CContext * pContext = 0) const;
	CHART * Replace(const CHART * tbstring, int string_length, const CHART * replaceto, int to_length, int & result_length, int start = -1, int ntimes = -1, MatchResult * result = 0, CContext * pContext = 0) const;
	int GetNamedGroupNumber(const CHART * group_name) const;

public:
	static void ReleaseString (CHART    * tbstring );
...
template <class CHART> inline MatchResult CRegexpT <CHART> :: MatchExact(const CHART * tbstring, CContext * pContext) const
{
	return MatchExact(tbstring, CBufferRefT<CHART>(tbstring).GetSize(), pContext);
}

template <class CHART> MatchResult CRegexpT <CHART> :: MatchExact(const CHART * tbstring, int length, CContext * pContext) const
{
	...
	pContext->m_pMatchString  = (void*)tbstring;
...
template <class CHART> MatchResult CRegexpT <CHART> :: Match(const CHART * tbstring, int start, CContext * pContext) const
{
	return Match(tbstring, CBufferRefT<CHART>(tbstring).GetSize(), start, pContext);
}

template <class CHART> MatchResult CRegexpT <CHART> :: Match(const CHART * tbstring, int length, int start, CContext * pContext) const
{
	if(m_builder.m_pTopElx == 0)
		return 0;
	...
	PrepareMatch(tbstring, length, start, pContext);
...
template <class CHART> inline CContext * CRegexpT <CHART> :: PrepareMatch(const CHART * tbstring, int start, CContext * pContext) const
{
	return PrepareMatch(tbstring, CBufferRefT<CHART>(tbstring).GetSize(), start, pContext);
}

template <class CHART> CContext * CRegexpT <CHART> :: PrepareMatch(const CHART * tbstring, int length, int start, CContext * pContext) const
{
	if(m_builder.m_pTopElx == 0)
		return 0;
...
template <class CHART> CHART * CRegexpT <CHART> :: Replace(const CHART * tbstring, const CHART * replaceto, int start, int ntimes, MatchResult * result, CContext * pContext) const
{
	int result_length = 0;
	return Replace(tbstring, CBufferRefT<CHART>(tbstring).GetSize(), replaceto, CBufferRefT<CHART>(replaceto).GetSize(), result_length, start, ntimes, result, pContext);
}

template <class CHART> CHART * CRegexpT <CHART> :: Replace(const CHART * tbstring, int string_length, const CHART * replaceto, int to_length, int & result_length, int start, int ntimes, MatchResult * remote_result, CContext * oContext) const
{
	...
	// prepare
	pContext  = PrepareMatch(tbstring, string_length, start, pContext);
	lastIndex = beginpos;
	...
	for(ntime = 0; ntimes < 0 || ntime < ntimes; ntime ++)
	{
		...
		if( rightleft )
		{
			...
			if( distance )
			{
				buffer.Push(tbstring + result->GetEnd());
		...
		else
		{
			...
			if( distance )
			{
				buffer.Push(tbstring + lastIndex);
				buffer.Push((const CHART *)distance);
		...
		for(i=tb; i!=te; i+=ts)
		{
			...
			if( off == -1 )
			{
				...
				case RCHART('+'):
					...
					sub = tbstring + result->GetGroupStart(nmatch);
			...
			{
				sub = tbstring + result->GetGroupStart(len);
...
template <class CHART> inline void CRegexpT <CHART> :: ReleaseString(CHART * tbstring)
{
	if(tbstring != 0) free(tbstring);
}
