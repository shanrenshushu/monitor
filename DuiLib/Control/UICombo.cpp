...
void CComboWnd::OnFinalMessage(HWND hWnd)
{
    ...
    m_pOwner->Invalidate();
	if(pOldListItem)
		pOldListItem->SetWillSelect(false);
    delete this;
}
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
		...
		{
			//m_pOwner->GetManager()->SetFocusNeeded(m_pOwner);
			m_pOwner->Invalidate();
			m_pOwner->GetManager()->SendNotify(m_pOwner, DUI_MSGTYPE_COMBOACITVE);
		}
    }
        ...
        case VK_ESCAPE:
			...
			break;
        case VK_RETURN:
			m_pOwner->SelectItem(m_WillSel, true);
	...
	{
		if (!isAlreadySet && m_iOldSel>=0 && WM_PAINT == uMsg)
		{
			isAlreadySet = true;
			EnsureVisible(m_iOldSel);
		}
...
void CComboWnd::WillSelNext()
{
	int index = m_pLayout->FindSelectable(m_WillSel+1,true);
	if (index<0)
		return;
	
	CControlUI *pItem = m_pLayout->GetItemAt(m_WillSel+1);
	if (pItem)
	{
		IListItemUI* pListItem = static_cast<IListItemUI *>(pItem->GetInterface(DUI_CTR_LISTITEM));
		if (pListItem)
		{
			if(pOldListItem)
				pOldListItem->SetWillSelect(false);
			pListItem->SetWillSelect(true);
			pOldListItem = pListItem;
			m_WillSel++;
		}
	}
}

void CComboWnd::WillSelLast()
{
	int index = m_pLayout->FindSelectable(m_WillSel-1,false);
	if (index<0)
		return;
...
CDuiString CComboUI::GetClass() const
{
    return _T("ComboUI");
}
...
void CComboUI::DoEvent(TEventUI& event)
{
    ...
    if( event.Type == UIEVENT_KEYDOWN )
    {
        ...
        case VK_UP:
			...
			{
				m_pWindow->m_pLayout->LineUp();
				m_pWindow->WillSelLast();
				m_pWindow->EnsureVisible(m_pWindow->m_WillSel);
			}
        ...
        case VK_DOWN:
			if (m_pWindow && m_pWindow->m_pLayout)
			{
				m_pWindow->m_pLayout->LineDown();
				m_pWindow->WillSelNext();
				m_pWindow->EnsureVisible(m_pWindow->m_WillSel);
			}
...
void CComboUI::SetText(LPCTSTR text)
{
	if( text == 0 ) return ;
	for (int i=0;i<m_items.GetSize();i++)
	{
		CControlUI* pControl = static_cast<CControlUI*>(m_items[i]);
		CDuiString temp= pControl->GetText();
		if (temp == text)
		{
			//m_iCurSel = i;
			SelectItem(i);
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 ) {
        ...
        if( pElement != NULL ) {
			 if (IsFocused() && m_IsTabControl)
			 {
				 RECT temp;
				 temp.left = rcText.left+2;
				 temp.right = rcText.right-18;
				 temp.top = rcText.top+2;
				 temp.bottom = rcText.bottom-2;
				 CRenderEngine::DrawColor(hDC, temp, 0xFF1E90DD);
			 }
			 rcText.left += 5;
...
void CComboUI::GetTabControl( vector<CControlUI* > &TempArr,UINT uFlags )
{
	if(!m_IsTabControl) return;
