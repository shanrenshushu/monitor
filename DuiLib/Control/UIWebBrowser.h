...
namespace DuiLib
{
	...
	public:
		...
		void SetNewWindowEnable(bool enable){m_bNewWindowEnable = enable;}
		...
		void GetApplication(IDispatch** pDisp);
		void GetHtmlDocument(IDispatch** pDisp);
	...
	protected:
		...
		void NewWindow2(IDispatch **pDisp, VARIANT_BOOL *&Cancel);
		...
		bool FindIconInHead(IDispatch *pDisp,wstring &out);
		bool FindIconPath(wchar_t *p,wstring &out);
		bool RegexMatch(const wchar_t *p,wchar_t *reg,wstring &out);
	public:
		IWebBrowser2*			m_pWebBrowser2; //浏览器指针
		void *mpCustomData;
		CEventSource *OnEvent;
		int mIsDel;
		wstring mFaviconPath;
