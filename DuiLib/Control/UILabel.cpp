...
namespace DuiLib
{
	...
	CDuiString CLabelUI::GetClass() const
	{
		return _T("LabelUI");
	}
	...
	void CLabelUI::SetHotFont(int index)
	{
		m_iHotFont = index;
		Invalidate();
	}
	...
	void CLabelUI::SetBkImage(LPCTSTR pStrImage)
	{
	...
	void CLabelUI::SetNormalGifFile( LPCTSTR pstrName )
	{
	...
	void CLabelUI::SetVisible( bool bVisible /*= true*/ )
	{
		if(bVisible == false && m_pGif)
		{
			m_pManager->KillTimer(this, GIF_TIMER_ID);
		}
	...
	void CLabelUI::PaintStatusImage( HDC hDC )
	{
		if(m_pGif)
		{
			if (m_isUpdateTime)//gif图片只和定时器有关，不然gif帧率过快过慢
			{
