...
CListUI::CListUI() : m_pCallback(NULL), m_bScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1),m_iCurSelActivate(-1),m_bKeyDownUsed(true)
{
...
CDuiString CListUI::GetClass() const
{
    return _T("ListUI");
}
...
void CListUI::DoEvent(TEventUI& event)
{
    ...
    case UIEVENT_KEYDOWN:        
		if (m_bKeyDownUsed)   //设置keydown变量判断是否使用默认键盘处理 
		{
			switch( event.chKey ) {
			case VK_UP:
...
void CListUI::SetKeyDownUsed( bool bflag /*= false*/ )
{
	m_bKeyDownUsed = bflag;
}
...
CDuiString CListHeaderUI::GetClass() const
{
...
CDuiString CListHeaderItemUI::GetClass() const
{
...
CDuiString CListElementUI::GetClass() const
{
...
CDuiString CListLabelElementUI::GetClass() const
{
...
CDuiString CListTextElementUI::GetClass() const
{
...
void CListTextElementUI::DrawItemText(HDC hDC, const RECT& rcItem)
{
    ...
    for( int i = 0; i < pInfo->nColumns; i++ )
    {
		...
		/*m_rcLinks[i].left = rcItem.left;
		m_rcLinks[i].right = rcItem.right;
		m_rcLinks[i].top = rcItem.top;
		m_rcLinks[i].bottom = rcItem.bottom;*/
...
CDuiString CListContainerElementUI::GetClass() const
{
...
{
	if( m_pOwner == NULL ) return;
	CControlUI *drawcontrol = GetDrawForComBoControl();
	if (!drawcontrol)
	{
		return;
	}
	CDuiString text = drawcontrol->GetText();
	...
	if( pInfo->bShowHtml )
		CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, text, iTextColor, \
		NULL, NULL, nLinks, DT_SINGLELINE | pInfo->uTextStyle);
	else
		CRenderEngine::DrawText(hDC, m_pManager, rcText, text, iTextColor, \
		pInfo->nFont, DT_SINGLELINE | pInfo->uTextStyle);
    return;
}
