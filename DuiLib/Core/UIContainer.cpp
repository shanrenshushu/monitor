...
namespace DuiLib
{
	...
	CDuiString CContainerUI::GetClass() const
	{
	...
	void CContainerUI::RemoveAllSafe()
	{
		for( int it = 0; m_bAutoDestroy && it < m_items.GetSize();  ) 
		{
			...
			{
				CControlUI *tem = static_cast<CControlUI*>(m_items[it]);
				m_items.Remove(it);
				delete tem;
			}
		}
	...
	void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("onlydrawchild")) == 0 ) SetOnlyDrawChild(_tcscmp(pstrValue, _T("true")) == 0);
	...
	void CContainerUI::GetTabControl( vector<CControlUI*> &TempArr,UINT uFlags )
	{
		...
		for (int i=0;i<m_items.GetSize();i++)
		{
			...
			pControl->GetTabControl(TempArr,uFlags);
		}
	}

	CControlUI* CContainerUI::GetDrawForComBoControl()
	{
		for (int i=0;i<m_items.GetSize();i++)
		{
			CControlUI* pControl = static_cast<CControlUI*>(m_items[i]);
			if (pControl->IsDrawForCombo())
			{
				return pControl;
			}
			else
			{
				CControlUI* pControl2 = pControl->GetDrawForComBoControl();
				if(pControl2)
					return pControl2;
			}
		}
