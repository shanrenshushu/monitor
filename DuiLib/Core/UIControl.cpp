...
CDuiString CControlUI::GetClass() const
{
    return _T("ControlUI");
}
...
void CControlUI::SetNetworkImage(LPCTSTR pStrImage, LPCTSTR pStrMD5)
{
	CDuiString url_img = pStrImage;
	std::regex_constants::syntax_option_type option_type = std::regex_constants::icase;
	std::wregex wszPattern(L"http://[^\\\"\\>\\<]+?\\.(png|jpg|bmp|gif)", option_type);
	std::wsmatch wszMatchResult;
	bool bUrlImgMatch = std::regex_search(std::wstring(pStrImage), wszMatchResult, wszPattern);

	if(bUrlImgMatch)
	{
		if( m_pManager != NULL ) 
			m_pManager->SendNotify(this, DUI_MSGTYPE_DOWNLOADIMG, (WPARAM)pStrImage, (LPARAM)pStrMD5);
	}
}
...
{
	if(_tcsstr(m_sBkImage, _T(".ico"))|| _tcsstr(m_sBkImage, _T(".ICO")))
	{
		HICON hIcon = (HICON)::LoadImage(m_pManager->GetInstance(), pStrImage, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR | LR_LOADFROMFILE | LR_DEFAULTSIZE);  
		if (hIcon == NULL)  
			hIcon = (HICON)::LoadImage(m_pManager->GetInstance(), m_pManager->GetResourcePath() + pStrImage, IMAGE_ICON, 0, 0, LR_DEFAULTCOLOR | LR_LOADFROMFILE | LR_DEFAULTSIZE);  
		if (NULL != hIcon)  
		{      
			::DrawIconEx(hDC, m_rcPaint.left, m_rcPaint.top, hIcon, m_rcPaint.right - m_rcPaint.left,  m_rcPaint.bottom - m_rcPaint.top, 0, NULL, DI_NORMAL);  
			::DeleteObject(hIcon); 
			return true;
		}  
		return false;
...
void CControlUI::SetVisibleEx(bool bVisible)
{
	if( m_bVisible == bVisible ) return;
...
void CControlUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("tabindex")) == 0 ) SetTabIndex(_ttoi(pstrValue));
	else if( _tcscmp(pstrName, _T("drawforcombo")) == 0 ) SetDrawForCombo(_tcscmp(pstrValue, _T("true")) == 0);
}
...
void CControlUI::GetTabControl( vector<CControlUI*> &TempArr,UINT uFlags )
{
