...
CGifHandler* CRenderEngine::LoadGif( STRINGorID bitmap, LPCTSTR type /*= NULL*/, DWORD mask /*= 0*/ )
{
	...
	if (!pData)
	{
		//::MessageBox(0, _T("读取GIF图片数据失败！"), _T("抓BUG"), MB_OK);
		return NULL;
	}
    ...
    if( lpAlphaBlend && (alphaChannel || uFade < 255) ) {
        ...
        if( !hole ) {
            ...
            if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
				...
				{
					//图片无缩放，平铺满控件区域
					//由于项目中几乎不用该功能， 改为图片无缩放居中显示
					LONG lWidth = rcBmpPart.right - rcBmpPart.left;
					LONG lHeight = rcBmpPart.bottom - rcBmpPart.top;
					lpAlphaBlend(hDC, rcDest.left + (rcDest.right - rcDest.left - lWidth) / 2, rcDest.top + (rcDest.bottom - rcDest.top - lHeight) / 2, \
						lWidth, lHeight, hCloneDC, rcBmpPart.left, rcBmpPart.top, lWidth, lHeight, bf);
            ...
            if( !hole ) {
                ...
                if( ::IntersectRect(&rcTemp, &rcPaint, &rcDest) ) {
                    ...
                    else if( xtiled && ytiled ) 
					{
						...
						LONG lWidth = rcBmpPart.right - rcBmpPart.left;
						LONG lHeight = rcBmpPart.bottom - rcBmpPart.top;
						::BitBlt(hDC, rcDest.left + (rcDest.right - rcDest.left - lWidth) / 2, rcDest.top + (rcDest.bottom - rcDest.top - lHeight) / 2, \
							lWidth, lHeight, hCloneDC, rcBmpPart.left, rcBmpPart.top, SRCCOPY);
