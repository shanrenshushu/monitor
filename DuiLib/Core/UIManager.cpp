...
CPaintManagerUI::cNameCreatorMap CPaintManagerUI::mNameCreatorMap;
...
//#endif
	...
	case WM_PAINT:
		{
			...
			else
			{
				...
				///////////////////////////////////////
				/*int m_maxX=GetSystemMetrics(SM_CXSCREEN);
				int m_maxY=GetSystemMetrics(SM_CYSCREEN);
				...
				hDC = GetDC(m_hWndPaint);
				m_pRoot->DoPaint(m_hMemDC, ps.rcPaint);
    ...
    case WM_SIZE:
        {
            if( m_pFocus != NULL ) {
                ...
                m_pFocus->Event(event);
				//m_pFocus->SetFocus()
				//SetFocusNeeded(m_pFocus);
				//SendNotify(m_pFocus,DUI_MSGTYPE_SETFOCUS);
            }
            if( m_pRoot != NULL /*&& !m_pFocus*/) 
				m_pRoot->NeedUpdate();
        }
        return true;
    case WM_TIMER:
    ...
    case WM_LBUTTONDOWN:
        {
            ...
            //::SetFocus(m_hWndPaint);//----防止原焦点控件失去焦点----yinguo
			...
			//////////////////////////////////////////////////////////////////////////
			CDuiString classMame=pControl->GetClass();
			if( _tcscmp(classMame, _T("VerticalLayoutUI"))==0 || _tcscmp(classMame, _T("HorizontalLayoutUI"))==0){
				if(pControl->GetName().IsEmpty()) break;//防止点击空白处焦点控件失去焦点 -----yinguo
			}
			//////////////////////////////////////////////////////////////////////////
    ...
    case WM_LBUTTONDBLCLK:
        {
			//::SetFocus(m_hWndPaint);----yinguo禁止直接设置焦点
            POINT pt = { GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) };
            m_ptLastMousePos = pt;
			CControlUI* pControl = FindControl(pt);
			if (pControl == NULL)break;
	...
	//case WM_ACTIVATE:
		{
			if (wParam == WA_ACTIVE)
			{
				...
				if( m_pFocus) 
				{
					//DUI__Trace(_T("IN  this=0x%x,%s"),this,m_pFocus->GetName());
					//m_pPreFocus = m_pFocus;
					//SetFocus(NULL);
					SetFocusNeeded(m_pFocus);
				}
			}
			...
			{
				if( m_pFocus) 
					DUI__Trace(_T("OUT this=0x%x,%s"),this,m_pFocus->GetName());
				else
					DUI__Trace(_T("OUT this=0x%x,"),this);
			}
		}
		break;*/
    default:
...
void CPaintManagerUI::ReapObjects(CControlUI* pControl)
{
	...
	if( pControl == m_pFocusNeed ) m_pFocusNeed = NULL;
    KillTimer(pControl);
...
bool CPaintManagerUI::DeselectOptionGroup(LPCTSTR pStrGroupName)
{
	LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
	if( lp ) 
	{
...
void CPaintManagerUI::SelectFromOptionGroupByName(LPCTSTR pStrGroupName, LPCTSTR pstrName, bool bSelected, bool bOnlyChangeSelectedState)
{
	LPVOID lp = m_mOptionGroup.Find(pStrGroupName);
	if( lp ) 
	{
		...
		{
			COptionUI* pControl = static_cast<COptionUI*>(aOptionGroup->GetAt(i));
			if(_tcscmp(pControl->GetName(), pstrName) == 0)
			{
				pControl->Selected(bSelected, bOnlyChangeSelectedState);
				break;
			}
		}
	}
...
void CPaintManagerUI::SetFocusNeeded(CControlUI* pControl)
{
	...
	//ClearFocus();
    m_bFocusNeeded = true;
	m_pFocusNeed = pControl;
    if( m_pRoot != NULL ) m_pRoot->NeedUpdate();
}

void CPaintManagerUI::ClearFocus()
{
	m_pFocus = NULL;
}
...
bool SortByTabIndexUp(CControlUI* left,CControlUI* right)
{
	return left->GetTabIndex() < right->GetTabIndex();
}

bool CPaintManagerUI::SetNextTabControl(bool bForward/* = true*/)
{
...
#else
	vector<CControlUI*> TempArr;
	m_pRoot->GetTabControl(TempArr,UIFIND_VISIBLE | UIFIND_ENABLED);
	::sort(TempArr.begin(),TempArr.end(),SortByTabIndexUp);
	CControlUI* pControl = 0;
	int findpos=-1;
	for (int i=0;i<TempArr.size();i++)
	{
		if (TempArr[i] == m_pFocus)
		{
			findpos = i;
			break;
		}
	...
	if (bForward)
	{
		for (int i=findpos+1;i<TempArr.size();i++)
		{
			pControl = TempArr[i]->FindControl(__FindControlFromTabEx, &info1, UIFIND_ME_FIRST | UIFIND_VISIBLE | UIFIND_ENABLED);
			if (pControl)
			{
				break;
			}
	...
	else
	{
		...
		for (int i=findpos-1;i>=0 && TempArr.size()>0;i--)
		{
			pControl = TempArr[i]->FindControl(__FindControlFromTabEx, &info1, UIFIND_ME_FIRST | UIFIND_VISIBLE | UIFIND_ENABLED);
			if (pControl)
			{
		...
		if (pControl ==0)
		{
			for (int i=TempArr.size()-1;i>findpos&&i>=0;i--)
			{
				pControl = TempArr[i]->FindControl(__FindControlFromTabEx, &info1, UIFIND_ME_FIRST | UIFIND_VISIBLE | UIFIND_ENABLED);
				if (pControl)
				{
	...
	{
		SetFocus(pControl);
		if(_tcscmp(pControl->GetClass(), _T("EditUI")) == 0 )
		{
			static_cast<CEditUI *>(pControl)->SetSelAll();
		}
	}
    return true;
}
...
{
    const TImageInfo* data = static_cast<const TImageInfo*>(m_mImageHash.Find(bitmap));
    if( !data ) 
	{
        data = AddImage(bitmap, type, mask);
    }
...
{
    ASSERT(m_pRoot);
    return m_pRoot->FindControl(__FindControlFromPoint, &pt, UIFIND_ENABLED| UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST);
}
...
CControlUI* CPaintManagerUI::FindSubControlByPoint(CControlUI* pParent, POINT pt) const
{
    ...
    ASSERT(pParent);
    return pParent->FindControl(__FindControlFromPoint, &pt, UIFIND_ENABLED | UIFIND_VISIBLE | UIFIND_HITTEST | UIFIND_TOP_FIRST);
}
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromTab(CControlUI* pThis, LPVOID pData)
{
	...
	/* Begin : 由于项目需要及个人能力所限，   
	 * __FindControlFromTab 函数暂时只支持 EditUI控件 
	 */
    //if( (pThis->GetControlFlags() & UIFLAG_TABSTOP) == 0 ) return NULL;	
	if(_tcscmp(pThis->GetClass(), _T("EditUI")) != 0 ) return NULL;	
	/* End */
    pInfo->pLast = pThis;
...
CControlUI* CALLBACK CPaintManagerUI::__FindControlFromTabEx(CControlUI* pThis, LPVOID pData)
{
    return pThis;
}
...
void CPaintManagerUI::RegisterNodeClass( const wstring& name, cNodeCreator creator )
{
	if (!mNameCreatorMap.insert(cNameCreatorMap::value_type(name, creator)).second)
	{
		assert(0 && "注册控件失败");
	}
}

bool CPaintManagerUI::NodeClassExists( const wstring& name )
{
	cNodeCreator creator;
	cNameCreatorMap::iterator i = mNameCreatorMap.find(name);
	if (i == mNameCreatorMap.end())
	{
		return false;
	}
	return true;
...
CControlUI* CPaintManagerUI::CreateControl( const std::wstring& name )
{
	cNodeCreator creator;
	cNameCreatorMap::iterator i = mNameCreatorMap.find(name);
	if (i == mNameCreatorMap.end())
	...
	CControlUI* pContrul = (CControlUI*) creator();
	return pContrul ;
}
