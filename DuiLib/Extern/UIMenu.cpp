...
namespace DuiLib {
    ...
    CDuiString CMenuUI::GetClass() const {
    ...
    bool CMenuUI::SelectItemActivate(int iIndex) {
        return false;
    }
    ...
    static int MINITEMWIDTH = 95;
    static int MINITEMHEIGHT = 30;
    ...
    void CMenuWnd::Init(CMenuElementUI *pOwner, LPCTSTR xml, LPCTSTR pSkinType, CDuiRect rc) {
        m_BasedPoint.x = rc.left;
        m_BasedPoint.y = rc.top;
        MINITEMWIDTH = rc.GetWidth();
        MINITEMHEIGHT = rc.GetHeight();
        m_pOwner = pOwner;
        m_pLayout = NULL;
    ...
    bool CMenuWnd::InitMenuElem(CMenuUI *pOwner, STRINGorID xml, LPCTSTR pSkinType, int count) {
        if((!pOwner) || (!xml.m_lpstr) || (count <= 0))
            return false;

        CDialogBuilder builder;
        CMenuBuilderCallback menuCallback;
        CControlUI *pMenuElem =  builder.Create(xml, pSkinType, &menuCallback, &m_pm);
        if( pMenuElem != NULL ) {
            for(int i = 0; i < count; ++i) {
                if( pMenuElem == NULL ) {
                    if(builder.GetMarkup()->IsValid()) {
                        pMenuElem = builder.Create( &menuCallback, &m_pm, 0);
                    } else
                        pMenuElem = builder.Create(xml, pSkinType, &menuCallback, &m_pm);
                }
                if( pMenuElem != NULL ) {
                    pOwner->Add(pMenuElem);
                    pMenuElem = NULL;
                }
            }
    ...
    void CMenuWnd::resize(CControlUI *pRoot) {
		if (!pRoot)
		{
			return;
		}
        ...
        int pmWidth = m_pm.GetClientSize().cx;
        pmWidth = (pmWidth > szAvailable.cx ? pmWidth : szAvailable.cx);
        int pmHeight = m_pm.GetClientSize().cy;
        m_pm.SetInitSize(pmWidth, szAvailable.cy);
        ...
        if(rc.GetWidth() < MINITEMWIDTH)
            rc.right = rc.left + MINITEMWIDTH;
        //if(rc.GetHeight() < MINITEMHEIGHT)
        //rc.bottom = rc.top + MINITEMHEIGHT;
        if(rc.GetHeight() < 1)
            rc.bottom = rc.top + 1;
    ...
    LRESULT CMenuWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam) {
        if( uMsg == WM_CREATE ) {
            if( m_pOwner != NULL) {
                ...
                m_pLayout->SetBorderColor(0xFFC6E2FF);
                ...
                CControlUI *pRoot = builder.Create(m_xml.GetData(), m_sType.GetData(), &menuCallback, &m_pm);
				ASSERT(pRoot);
    ...
    CDuiString CMenuElementUI::GetClass() const
	{
    ...
    void CMenuElementUI::DoEvent(TEventUI &event) {
        ...
        if( event.Type == UIEVENT_BUTTONDOWN ) {
            ...
            //gmenusel= event.pSender->GetName();
            gmenusel = event.pSender->GetTag();
			CDuiString strUser = event.pSender->GetUserData();
			int len = strUser.GetLength();
			if (len > 0)
			{
				TCHAR* pdata = new TCHAR [len +1];
				_tcsncpy_s(pdata,len+1,strUser.GetData(),len);
				PostMessage(gparendHwnd, WM_MENUSELECTMSG, (WPARAM)gmenusel, (LPARAM)pdata);
			}
			else
			{
				PostMessage(gparendHwnd,WM_MENUSELECTMSG,(WPARAM)gmenusel,0);
			}
            return;
        }
    ...
    void CMenuElementUI::CreateMenuWnd() {
        ...
        m_pWindow->Init(static_cast<CMenuElementUI *>(this), _T(""), _T(""), CDuiRect());
    }
